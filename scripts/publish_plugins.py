...
for platform in platforms:
    python_bin_dir = f'{build_path}/{platform}/python'
...
for platform in platforms:
    zip_file = f"{platform}/dcos-core-cli.zip"
    s3_client.upload_file(
        f"{build_path}/{zip_file}",
        "downloads.dcos.io",
        bucket_key_pattern.format(stability, platform, version),
    )
    ...
    if os.environ.get("TAG_NAME"):
        ...
        s3_client.upload_file(
            f"{build_path}/{zip_file}",
            "downloads.dcos.io",
            bucket_key_pattern.format(
                stability, platform, version.rsplit('.', 1)[0] + '.latest'
            ),
        )
